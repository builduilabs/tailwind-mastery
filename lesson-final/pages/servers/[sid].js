export default function Server() {
  return (
    <>
      <div className="flex flex-col bg-gray-800 w-60">
        <button className="flex items-center h-12 px-4 font-semibold text-white transition shadow-sm font-title hover:bg-gray-550/[0.16]">
          <div className="flex items-center justify-between w-full">
            <div className="flex items-center group">
              <div className="relative w-4 h-4 mr-1">
                <VerifiedIcon className="absolute inset-0 w-4 h-4 text-gray-550" />
                <CheckIcon className="absolute inset-0 w-4 h-4 text-white" />
              </div>
              <span className="text-sm">Tailwind CSS</span>
            </div>

            <ChevronIcon className="w-[18px] h-[18px] text-white opacity-80" />
          </div>
        </button>

        <div className="flex-1 mt-4 overflow-y-scroll font-medium text-gray-300">
          <div className="px-2 py-px">
            <a
              href="#"
              className="flex items-center text-gray-300 px-2 py-1 hover:bg-gray-550/[0.16] hover:text-gray-100 rounded group"
            >
              <BookIcon className="w-5 h-5 mr-1.5 text-gray-400" />
              <span>welcome</span>
              <AddPersonIcon className="invisible w-4 h-4 ml-auto text-gray-200 group-hover:visible hover:text-gray-100" />
            </a>
          </div>
          <div className="px-2 py-px">
            <a
              href="#"
              className="flex items-center text-gray-300 px-2 py-1 hover:bg-gray-550/[0.16] hover:text-gray-100 rounded group"
            >
              <SpeakerphoneIcon className="w-5 h-5 mr-1.5 text-gray-400" />
              <span>announcements</span>
              <AddPersonIcon className="invisible w-4 h-4 ml-auto text-gray-200 group-hover:visible hover:text-gray-100" />
            </a>
          </div>

          <div className="mt-4 pt-0.5">
            <div className="px-0.5 py-0.5">
              <button className="flex items-center w-full pr-2 text-xs font-semibold text-gray-300 uppercase hover:text-gray-100">
                <ArrowComponent className="w-3 h-3 mr-0.5" />
                <span className="tracking-wide font-title">Tailwind CSS</span>
              </button>
            </div>
          </div>

          <div className="space-y-0.5 mt-0.5 pt-px px-2">
            <a
              href="#"
              className="flex items-center py-1 hover:bg-gray-550/[0.16] rounded"
            >
              <HashtagIcon className="w-5 h-5 ml-2 mr-1.5 text-gray-400" />
              <span className="text-white">general</span>
            </a>
            <a
              href="#"
              className="flex items-center py-1 hover:bg-gray-550/[0.16] rounded"
            >
              <HashtagIcon className="w-5 h-5 ml-2 mr-1.5 text-gray-400" />
              <span className="text-white">plugins</span>
            </a>
            <a
              href="#"
              className="flex items-center py-1 hover:bg-gray-550/[0.16] rounded"
            >
              <HashtagIcon className="w-5 h-5 ml-2 mr-1.5 text-gray-400" />
              <span className="text-gray-300">help</span>
            </a>
            <a
              href="#"
              className="flex items-center py-1 bg-gray-550/[0.32] rounded"
            >
              <HashtagIcon className="w-5 h-5 ml-2 mr-1.5 text-gray-400" />
              <span className="text-white">internals</span>
            </a>
          </div>
        </div>
      </div>

      <div className="flex flex-col flex-1 bg-gray-700">
        <div className="flex items-center h-12 px-3 shadow-sm">general</div>
        <div className="flex-1 p-3 space-y-4 overflow-y-scroll">
          {[...Array(40)].map((_, i) => (
            <p key={i}>
              Message {i}. Lorem ipsum dolor sit amet consectetur adipisicing
              elit. Vel saepe laudantium sed reprehenderit incidunt! Hic rem
              quos reiciendis, fugit quae ratione beatae veniam laborum
              voluptatem, iusto dolorum, voluptates suscipit quia.
            </p>
          ))}
        </div>
      </div>
    </>
  );
}

function VerifiedIcon(props) {
  return (
    <svg
      aria-label="Verified"
      aria-hidden="false"
      viewBox="0 0 16 15.2"
      {...props}
    >
      <path
        fill="currentColor"
        fillRule="evenodd"
        d="M16 7.6c0 .79-1.28 1.38-1.52 2.09s.44 2 0 2.59-1.84.35-2.46.8-.79 1.84-1.54 2.09-1.67-.8-2.47-.8-1.75 1-2.47.8-.92-1.64-1.54-2.09-2-.18-2.46-.8.23-1.84 0-2.59S0 8.39 0 7.6s1.28-1.38 1.52-2.09-.44-2 0-2.59 1.85-.35 2.48-.8S4.78.28 5.53 0 7.2.83 8 .83s1.75-1 2.47-.8.91 1.64 1.53 2.09 2 .18 2.46.8-.23 1.84 0 2.59S16 6.81 16 7.6z"
      />
    </svg>
  );
}

function CheckIcon(props) {
  return (
    <svg aria-hidden="false" viewBox="0 0 16 15.2" {...props}>
      <path
        d="M7.4 11.17L4 8.62l1-1.36 2 1.53L10.64 4 12 5z"
        fill="currentColor"
      />
    </svg>
  );
}

function ChevronIcon(props) {
  return (
    <svg viewBox="0 0 18 18" fill="none" {...props}>
      <path
        d="M5.20711 5.54289C4.81658 5.15237 4.18342 5.15237 3.79289 5.54289C3.40237 5.93342 3.40237 6.56658 3.79289 6.95711L5.20711 5.54289ZM9 10.75L8.29289 11.4571C8.68342 11.8476 9.31658 11.8476 9.70711 11.4571L9 10.75ZM14.2071 6.95711C14.5976 6.56658 14.5976 5.93342 14.2071 5.54289C13.8166 5.15237 13.1834 5.15237 12.7929 5.54289L14.2071 6.95711ZM3.79289 6.95711L8.29289 11.4571L9.70711 10.0429L5.20711 5.54289L3.79289 6.95711ZM9.70711 11.4571L14.2071 6.95711L12.7929 5.54289L8.29289 10.0429L9.70711 11.4571Z"
        fill="currentColor"
      />
    </svg>
  );
}

function BookIcon(props) {
  return (
    <svg viewBox="0 0 40 40" {...props}>
      <path
        fill="currentColor"
        fillRule="evenodd"
        clipRule="evenodd"
        d="M33 34.583V7.5h2v29.167H9c-2.209 0-4-1.866-4-4.167v-25c0-2.301 1.791-4.167 4-4.167h22v27.084H9c-1.104 0-2 .931-2 2.083 0 1.151.896 2.083 2 2.083h24zM23.972 10l-8.083 7.991-3.18-3.147L10 17.506l5.889 5.827 10.778-10.666L23.972 10z"
      />
    </svg>
  );
}

function SpeakerphoneIcon(props) {
  return (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        d="M3.9 8.26H2v7.034h1.9V8.26zM19.1 4v1.127L4.85 8.264v9.854c0 .432.296.807.72.914l3.8.94a.954.954 0 00.99-.35l2.347-3.098L19.1 17.93v1.129H21V4h-1.9zM9.218 17.994l-2.468-.611V15.21l3.92.864-1.452 1.92z"
        fill="currentColor"
      />
    </svg>
  );
}

function ArrowComponent(props) {
  return (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        fill="currentColor"
        fillRule="evenodd"
        clipRule="evenodd"
        d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6-1.41-1.41z"
      />
    </svg>
  );
}

function AddPersonIcon(props) {
  return (
    <svg viewBox="0 0 16 16" {...props}>
      <path
        fill="currentColor"
        d="M14 2h2v1h-2v2h-1V3h-2V2h2V0h1v2zM6.5 8.007a2.5 2.5 0 000-5 2.5 2.5 0 000 5zM6.5 8.34c-3.24 0-5.5 1.647-5.5 4v.667h11v-.667c0-2.36-2.26-4-5.5-4z"
      />
    </svg>
  );
}

function HashtagIcon(props) {
  return (
    <svg viewBox="0 0 24 24" {...props}>
      <path
        fill="currentColor"
        fillRule="evenodd"
        clipRule="evenodd"
        d="M5.887 21a.5.5 0 01-.493-.587L6 17H2.595a.5.5 0 01-.492-.586l.175-1A.5.5 0 012.77 15h3.58l1.06-6H4.005a.5.5 0 01-.492-.586l.175-1A.5.5 0 014.18 7h3.58l.637-3.587A.5.5 0 018.889 3h.984a.5.5 0 01.493.587L9.76 7h6l.637-3.587A.5.5 0 0116.889 3h.984a.5.5 0 01.493.587L17.76 7h3.405a.5.5 0 01.492.586l-.175 1A.5.5 0 0120.99 9h-3.58l-1.06 6h3.405a.5.5 0 01.492.586l-.175 1a.5.5 0 01-.492.414H16l-.637 3.587a.5.5 0 01-.492.413h-.984a.5.5 0 01-.493-.587L14 17H8l-.637 3.587a.5.5 0 01-.492.413h-.984zM9.41 9l-1.06 6h6l1.06-6h-6z"
      />
    </svg>
  );
}
